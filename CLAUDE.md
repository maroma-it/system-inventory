...
## Platform semantics live elsewhere

This file documents the **pipeline** — how exports are parsed, merged, and rendered. It does not
document the **platform**. Trigger enums, condition-operator codes, action parameters, target
resolution, token syntax, `calc()`, and the shape of an importable workflow JSON are all in
**`workflow-engine-reference.md`** (published to `docs/docs.html` as the "Workflow Engine" tab).

Read it before changing anything in `parser.py` that decodes a workflow, and before hand-writing or
generating a workflow export. It also records which enum codes are still undecoded — `parser.py`
...
- **`options`** is present only on fields that carry a choice list (dropdowns and similar), parsed
  from `ExtraProperties.DataSourceValues` and ordered by `SortOrder`. Each entry is the value a
  workflow condition, Switch case, or `FieldAssignments` entry must compare against. `DataSourceValues`
  carries both a `Key` and a `Value` and **they can differ** (`Key: "Ready for Review"` /
  `Value: "Completed & Ready for Review"`); only `Value` is published, because that is the side the
  engine matches — verified across the corpus, where every disambiguating comparison matches `Value`
  and none match `Key`. Publishing `Key` would hand consumers the string that silently never matches.
  The key is omitted rather than emitted empty so the file does not grow by an empty array on each of
  ~10,800 fields.
